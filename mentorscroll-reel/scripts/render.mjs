#!/usr/bin/env node
/**
 * Headless render: Playwright captures WebGL frames → FFmpeg MP4 + WebM
 */
...
function startServer() {
  // symlink/copy assets into reel folder for static serving
...
async function main() {
  ...
  const mp4 = path.join(OUT_DIR, `mentorscroll_newtons_laws_${stamp}.mp4`)
  const webm = path.join(OUT_DIR, `mentorscroll_newtons_laws_${stamp}.webm`)
  const framesDir = path.join(OUT_DIR, `frames_newtons_laws_${stamp}`)
  ...
  const meta = await page.evaluate(() => ({
    ...
    fps: window.__MS.fps,
    animOk: window.__MS.animOk?.() ?? window.__MS_ANIM_OK,
    actions: window.__MS.actions?.() ?? [],
  }))
  console.log('actions:', meta.actions, 'animOk:', meta.animOk)
  if (!meta.animOk) {
    throw new Error('Animation scrub check failed — character would look frozen. Aborting render.')
  }
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    await page.locator('#c').screenshot({ path: framePath, type: 'png' })
  ...
  console.log('Encoding MP4 + WebM in parallel...')
  await Promise.all([
    runFfmpeg([
      '-y',
      '-framerate',
